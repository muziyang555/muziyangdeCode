...
<s:form action="Money_in_Update" method="post" onsubmit="return check(this)">
<table border="none">
		<tr><td colspan="2" >账单编号:<s:textfield name="mi.moneyinid" readonly="true"/></td></tr>
    	<tr><td colspan="2">支出类型:<s:select list="mitype" name="mi.type" /></td></tr>
    	<tr><td colspan="2">账户类型:<s:select list="miaccounttype" name="mi.accounttype" /></td></tr>
    	<tr><td colspan="2">时间:<input type="date" name="mi.time"  id="name"value="<s:date  name="mi.time" format="yyyy-MM-dd"/>"/></td></tr>
    	<tr><td colspan="2">备注:<s:textfield name="mi.note"  id="name"/></td></tr>
    	<tr><td colspan="2">金额:<s:textfield name="mi.money"  id="name"/></td></tr>
    	<tr><td><input class="button" type="submit" value="修改"/></td>
    	<td><input  type="button" value="返回" class="sgBtn" id="btn7"
		onclick= "window.location.href='moneyin.action'" >	</td></tr>			
</table>	
